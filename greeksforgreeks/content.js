 ...
 *
 * Runs in the isolated world on https://www.geeksforgeeks.org/problems/*
 * Receives messages from the page-context interceptor (injected-interceptor.js),
 * enriches problem metadata if needed, debounces duplicate submissions,
 * and sends the submission package to the background service worker.
 */
...
(() => {
  ...
  /* ──────────────────── Process and Send to Background ──────────────────── */
  ...
  async function handleAcceptedSubmission(rawPayload) {
    ...
    // Keep processedSignatures bounded to prevent memory leaks in long sessions
    ...
    console.log('[GfGSync-Mini] Sending verified submission to service worker:', submissionData);
      ...
      chrome.runtime.sendMessage(
        ...
        (response) => {
          if (chrome.runtime.lastError) {
            ...
            console.log('[GfGSync-Mini] Sync triggered successfully:', response);
          }
        }
      );
    } catch (err) {
