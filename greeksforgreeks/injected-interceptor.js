/**
 * GfGSync-Mini — Main World Interceptor & DOM Observer
 *
 * Injected into the GeeksforGeeks page context (MAIN world) to intercept
 * network calls (fetch / XHR) to practiceapi.geeksforgeeks.org and observe DOM.
...
(() => {
  ...
  console.log('[GfGSync-Mini] Main-world interceptor initialized.');
  ...
  function getCodeFromEditor() {
    ...
    // 4. DOM fallback for Monaco / CodeMirror lines
    try {
  ...
  function getLanguageFromPage() {
    ...
    // 2. Check DOM language selector button or select element
    try {
      const selectors = [
        '[data-cy="lang-select"]',
  ...
  function isAcceptedPayload(payload) {
    ...
    // Reject compile/run-only test runs if explicitly marked
    if (
      payload.is_run ||
    ...
    // Check status_code / status_text / result fields
    const candidates = [
      payload.status,
      payload.status_text,
    ...
    // GfG API returns { is_correct: true } or { status_code: 1, error_type: 0 }
    ...
    if (
      ...
      // Check if it's a submission result (has score/points/test_cases)
  ...
  function dispatchAcceptedSubmission(payload, source = 'network') {
    ...
    if (!code || code.trim().length === 0) {
      console.warn('[GfGSync-Mini] Submission accepted but code is empty, waiting 300ms...');
      setTimeout(() => {
  ...
  function checkAndProcessNetworkData(url, body) {
    ...
    // Filter out common irrelevant endpoints
